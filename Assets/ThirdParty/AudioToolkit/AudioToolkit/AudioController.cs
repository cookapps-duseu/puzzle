...
public class AudioController : MonoBehaviour, ISingletonMonoBehaviour // can not make DLL with SingletonMonoBehaviour
{
    ...
    public class AudioController : SingletonMonoBehaviour<AudioController>, ISerializationCallbackReceiver
    {
        ...
        /// </summary>
        public const string AUDIO_TOOLKIT_VERSION = "12.2";
        ...
        /// </remarks>
        /// <returns>
        /// The <c>AudioObject</c> of the played track, or <c>null</c> if no  track could be played.
        /// </returns>
        ...
        /// </remarks>
        /// <returns>
        /// The <c>AudioObject</c> of the played audio, or <c>null</c> if no track could be played.
        /// </returns>
        ...
        /// <summary>
        /// Return the currently playing AudioObject of a playlist.
        /// </summary>
        /// <returns>
        /// The <c>AudioObject</c> of the current channel, or <c>null</c> if no track is played.
        /// </returns>
        static public AudioObject GetCurrentPlaylistAudioObject( AudioChannelType channel = AudioChannelType.Music )
        {
            var channelData = GetAudioChannel( channel );
            if( channelData.IsPlaylistPlaying() )
            {
                return channelData.currentlyPlaying;
            }
            else
                return null;
        }
    ...
    void OnDestroy()
        ...
        public AudioObject PlayAudioSubItem( AudioSubItem subItem, float volume, Vector3 worldPosition, Transform parentObj, double delay, double startTime, bool playWithoutAudioObject, AudioObject useExistingAudioObj, double dspTime = 0, AudioChannelType channel = AudioChannelType.Default, float startVolumeMultiplier = 1 )
        {
            ...
            bool playWithZeroVolume = false;

            switch ( audioCategory.playWithZeroVolume )
            {
                case AudioCategory.PlayWithZeroVolumeOptions.DefaultFromAudioController:
                    playWithZeroVolume = subItemAudioController.PlayWithZeroVolume;
                    break;
                case AudioCategory.PlayWithZeroVolumeOptions.On:
                    playWithZeroVolume = true; break;
                case AudioCategory.PlayWithZeroVolumeOptions.Off:
                    playWithZeroVolume = false; break;
            }
...
#pragma warning disable 612
        ...
        public void OnAfterDeserialize()
        {
            if ( this == null )
            {
                //When changing a prefab in Project-View directly unity somehow calls this function twice
                //The second time this == "null" and all values are default (looks like a unity-bug)
                return;
            }
